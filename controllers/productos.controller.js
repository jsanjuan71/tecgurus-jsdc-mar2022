...
function getCardLayout( title, description, quantity, action, imagePath ) {
    const template = `
        <div class="col-3">
            <div class="card" style="max-width: 18rem; min-width: 10rem;">
                <img src="${imagePath}" class="card-img-top" height="200rem" alt="...">
                <div class="card-body">
                    <h5 class="card-title" style="height: 5rem;">${title}</h5>
...
productos.forEach( (producto, key) => {
    html += (key > 0 && key % 4 == 0) ? '</div><br /><div class="row align-items-start">' : '' ;
    html += getCardLayout( producto.nombre, producto.descripcion, producto.precio, "Add", `assets/images/products/${producto.imagen}` );
});
productosContainer.innerHTML = html + '</div>';
